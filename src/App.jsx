...
export default function App() {
      ...
      if (currentUser) {
        ...
        if (docSnap.exists()) {
          ...
          await setDoc(userRef, {
            coins: coins > 0 ? coins : 50, // Dá 50 moedas no primeiro login
  ...
  // --- CORREÇÃO: DETECÇÃO DE PAGAMENTO ---
  useEffect(() => {
    ...
    // 1. Espera a autenticação do Firebase estar pronta antes de fazer qualquer coisa.
    if (!isAuthReady) return;

    // 2. Se o pagamento foi aprovado, processa o crédito.
    if ((status === 'approved' || query.get('collection_status') === 'approved') && paymentId) {
       ...
       setToastMsg('🔄 Confirmando pagamento...');

       const pendingAmount = Number(localStorage.getItem('mp_pending_amount') || 0);
       ...
       if (!isNaN(amount) && amount > 0) {
          ...
          setTimeout(() => {
             ...
             handlePurchaseCoins(amount, true); // Chama a função de crédito
             setLastPaymentAmount(amount);
             setShowPaymentSuccess(true); // Mostra a tela de sucesso
             localStorage.removeItem('mp_pending_amount');
             window.history.replaceState({}, document.title, window.location.pathname);
          }, 1000);
       } else {
          console.warn("Pagamento aprovado, mas sem 'Referência Externa' (quantidade de moedas).");
          if (!localStorage.getItem(processedKey)) {
             showToast('Erro na configuração do Mercado Pago: "Código de Referência" ausente.');
          }
       }
    }
  }, [isAuthReady]); // A dependência do 'user' foi removida para corrigir a falha de detecção.
  ...
  const handlePurchaseCoins = async (amount, isSystemCredit = false) => {
    ...
    // Se for um clique na loja, redireciona para o Mercado Pago
    if (!isSystemCredit && paymentLinks[amount]) {
        localStorage.setItem('mp_pending_amount', String(amount));
        window.location.href = paymentLinks[amount];
        return;
    }

    // Se for um crédito automático (retorno do MP), aplica as moedas
    if (isSystemCredit) {
        if (user) {
          try {
          ...
          // Fallback para usuário anônimo (não deveria acontecer no fluxo normal)
  ...
  const checkAlreadyDrawn = (gameNumbers) => {
    if (realData && realData[activeLottery] && realData[activeLottery].allDraws) {
        return realData[activeLottery].allDraws.some(draw => 
            draw.length === gameNumbers.length && draw.every((num, index) => num === gameNumbers[index])
        );
    }
    return false;
  };
  ...
  const generateGames = () => {
    ...
    if (user) {
      updateDoc(doc(db, "users", user.uid), { coins: increment(-currentCost) });
    } else {
  ...
  const generateGameWithAI = async () => {
    ...
    if (user) {
      updateDoc(doc(db, "users", user.uid), { coins: increment(-aiCost) });
    } else {
  ...
  const handleSaveGame = (game) => {
    ...
    if (user) {
      updateDoc(doc(db, "users", user.uid), { savedGames: arrayUnion(newSavedGame) });
    } else {
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br ${currentConfig.theme} flex items-center justify-center p-4 sm:p-8 font-sans text-gray-100 transition-colors duration-700 overflow-x-hidden relative`}>
      ...
      <style>{`
        @keyframes mystic-pulse { 0%, 100% { box-shadow: 0 0 15px rgba(192, 38, 211, 0.3); text-shadow: 0 0 5px rgba(255,255,255,0.5); } 50% { box-shadow: 0 0 40px rgba(217, 70, 239, 0.8), inset 0 0 20px rgba(217, 70, 239, 0.5); text-shadow: 0 0 15px rgba(255,255,255,1); transform: scale(1.02); } }
        @keyframes mystic-shimmer { 0% { background-position: 0% 50%; } 100% { background-position: 200% 50%; } }
        @keyframes astral-float { 0%, 100% { transform: translateY(0px) scale(1); filter: hue-rotate(0deg); } 50% { transform: translateY(-15px) scale(1.05); filter: hue-rotate(30deg); } }
        @keyframes mediumnic-fade { 0%, 100% { opacity: 0.6; filter: blur(1px); } 50% { opacity: 1; filter: blur(0px); text-shadow: 0 0 20px rgba(232, 121, 249, 1); } }
        .bet-btn-mystic { background: linear-gradient(180deg, #d946ef 0%, #a21caf 50%, #701a75 100%); border: 1px solid #f5d0fe; box-shadow: 0 6px 0 #4a044e, 0 10px 20px rgba(0,0,0,0.5), inset 0 2px 4px rgba(255,255,255,0.4); color: white; transition: all 0.1s; }
        .bet-btn-mystic:active:not(:disabled) { transform: translateY(6px); box-shadow: 0 0px 0 #4a044e, 0 4px 6px rgba(0,0,0,0.4), inset 0 2px 4px rgba(255,255,255,0.4); }
        .bet-btn-stat { background: linear-gradient(180deg, #38bdf8 0%, #0284c7 50%, #075985 100%); border: 1px solid #bae6fd; box-shadow: 0 6px 0 #082f49, 0 10px 20px rgba(0,0,0,0.5), inset 0 2px 4px rgba(255,255,255,0.4); color: white; transition: all 0.1s; }
        .bet-btn-stat:active:not(:disabled) { transform: translateY(6px); box-shadow: 0 0px 0 #082f49, 0 4px 6px rgba(0,0,0,0.4), inset 0 2px 4px rgba(255,255,255,0.4); }
        .bet-btn-gold { background: linear-gradient(180deg, #fcd34d 0%, #f59e0b 50%, #b45309 100%); border: 1px solid #fef3c7; box-shadow: 0 6px 0 #78350f, 0 10px 20px rgba(0,0,0,0.5), inset 0 2px 4px rgba(255,255,255,0.6); color: #451a03; transition: all 0.1s; }
        .bet-btn-gold:active:not(:disabled) { transform: translateY(6px); box-shadow: 0 0px 0 #78350f, 0 4px 6px rgba(0,0,0,0.4), inset 0 2px 4px rgba(255,255,255,0.6); }
        .bet-btn-disabled { background: linear-gradient(180deg, #4b5563 0%, #374151 50%, #1f2937 100%); border: 1px solid #6b7280; box-shadow: 0 6px 0 #111827; color: #9ca3af; cursor: not-allowed; }
        .tab-chip { border-bottom: 4px solid transparent; }
        .tab-chip.active { transform: translateY(2px); border-bottom: 0px solid transparent; }
      `}</style>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-300">
            <div className="bg-gray-900 border border-purple-500/30 rounded-3xl w-full max-w-md relative shadow-[0_0_50px_rgba(168,85,247,0.15)] animate-in zoom-in-95 overflow-hidden">
                <div className="bg-gradient-to-b from-purple-500/10 to-transparent p-6 text-center border-b border-white/5 relative">
                    <button onClick={() => setShowReferral(false)} className="absolute top-4 right-4 text-gray-500 hover:text-white transition-colors bg-gray-800/50 p-2 rounded-full"><X className="w-4 h-4" /></button>
                    <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-purple-500/20 mb-3 shadow-[0_0_20px_rgba(168,85,247,0.3)]"><Gift className="w-8 h-8 text-purple-400" /></div>
                ...
                <div className="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <div className="flex shadow-inner">
                            ...
                            <button onClick={handleCopyReferralLink} className="bg-purple-600 hover:bg-purple-500 text-white px-5 rounded-r-xl transition-colors flex items-center justify-center font-bold"><Copy className="w-4 h-4 mr-2" /> Copiar</button>
                        </div>
                    </div>
                    <div className="pt-5 border-t border-gray-800 space-y-2">
                        ...
                        <button onClick={handleSimulateReferralSignup} className="w-full bg-gray-800 hover:bg-gray-700 border border-gray-700 text-gray-300 text-xs font-bold py-2.5 rounded-xl transition-colors">Simular Registo (+15 🪙)</button>
                        <button onClick={handleSimulateReferralPurchase} className="w-full bg-gray-800 hover:bg-gray-700 border border-gray-700 text-gray-300 text-xs font-bold py-2.5 rounded-xl transition-colors">Simular Compra (+100 🪙)</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-300">
            <div className="bg-gray-900 border border-gray-700 rounded-3xl w-full max-w-lg relative shadow-2xl animate-in zoom-in-95 overflow-hidden flex flex-col max-h-[80vh]">
                <div className="bg-gray-800 p-6 text-center border-b border-white/5 relative shrink-0">
                    <button onClick={() => setShowSavedGames(false)} className="absolute top-4 right-4 text-gray-500 hover:text-white transition-colors bg-gray-900/50 p-2 rounded-full"><X className="w-4 h-4" /></button>
                    <h2 className="text-xl font-black text-white flex items-center justify-center gap-2"><Save className="w-5 h-5 text-emerald-400" /> Meus Jogos Salvos</h2>
                      ...
                      savedGames.map((game) => (
                        <div key={game.id} className="bg-gray-950/50 border border-gray-800 rounded-xl p-3 flex justify-between items-center">
                           <div>
                              ...
                              <div className="text-sm font-mono font-bold text-emerald-400 tracking-wider">{game.numbers.map(n => n.toString().padStart(2, '0')).join(' - ')}</div>
                           </div>
                           <button onClick={() => handleDeleteGame(game.id)} className="p-2 text-gray-600 hover:text-red-400 transition-colors"><Trash2 className="w-4 h-4" /></button>
                        </div>
                      ))
                    )}
                </div>
                <div className="p-4 border-t border-gray-800 bg-gray-900 shrink-0">
                    <button onClick={handleSimulateDrawCheck} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-2"><Bell className="w-4 h-4" /> Simular Conferência de Sorteio</button>
                </div>
            </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-300">
            <div className="bg-gray-900 border border-amber-500/30 rounded-3xl w-full max-w-md relative shadow-[0_0_50px_rgba(251,191,36,0.15)] animate-in zoom-in-95 overflow-hidden">
                <div className="bg-gradient-to-b from-amber-500/10 to-transparent p-6 text-center border-b border-white/5 relative">
                    <button onClick={() => setShowStore(false)} className="absolute top-4 right-4 text-gray-500 hover:text-white transition-colors bg-gray-800/50 p-2 rounded-full"><X className="w-4 h-4" /></button>
                    <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-amber-500/20 mb-3 shadow-[0_0_20px_rgba(251,191,36,0.3)]"><ShoppingCart className="w-8 h-8 text-amber-400" /></div>
                    <h2 className="text-2xl font-black text-white">Loja de Moedas da Sorte</h2>
                    <p className="text-xs text-gray-400 mt-2 font-medium">Adquira moedas virtuais para gerar jogos com Inteligência Artificial.</p>
                    <div className="mt-3 bg-amber-900/40 border border-amber-500/30 p-2 rounded-lg">
                        <p className="text-[10px] text-amber-200 font-bold leading-tight">⚠️ Importante: Após pagar, clique no botão <span className="text-white underline">"Voltar para o site"</span> no Mercado Pago para validar os créditos.</p>
                    </div>
                </div>
                <div className="p-6 space-y-3">
                    <div className="flex items-center justify-between p-4 rounded-2xl border border-gray-700 bg-gray-800/50 hover:bg-gray-800 hover:border-amber-500/50 transition-all cursor-pointer group" onClick={() => handlePurchaseCoins(100)}>
                        ...
                        <button className="bg-gray-900 text-white text-xs font-black px-4 py-2 rounded-xl border border-gray-700 flex items-center group-hover:bg-amber-500 group-hover:text-amber-950 group-hover:border-amber-400 transition-colors">R$ 4,90</button>
                    </div>
                    <div className="flex items-center justify-between p-4 rounded-2xl border border-amber-500/30 bg-amber-900/10 hover:bg-amber-900/20 hover:border-amber-400 transition-all cursor-pointer group relative overflow-hidden" onClick={() => handlePurchaseCoins(500)}>
                        <div className="absolute top-0 right-0 bg-amber-500 text-amber-950 text-[8px] font-black px-2 py-0.5 rounded-bl-lg uppercase tracking-widest shadow-sm">Mais Popular</div>
                        <div className="flex items-center">
                        ...
                        <button className="bg-gradient-to-r from-amber-500 to-yellow-400 text-amber-950 text-xs font-black px-4 py-2 rounded-xl border border-yellow-300 flex items-center shadow-lg group-hover:scale-105 transition-transform">R$ 24,50</button>
                    </div>
                    <div className="flex items-center justify-between p-4 rounded-2xl border border-purple-500/50 bg-purple-900/20 hover:bg-purple-900/30 hover:border-purple-400 transition-all cursor-pointer group relative overflow-hidden shadow-[0_0_15px_rgba(168,85,247,0.15)]" onClick={() => handlePurchaseCoins(2000)}>
                        {promoTimeLeft > 0 && (<div className="absolute top-0 right-0 bg-red-600 text-white text-[8px] font-black px-2.5 py-0.5 rounded-bl-lg uppercase tracking-widest shadow-md flex items-center animate-pulse">🔥 10% OFF • ⏳ {formatTime(promoTimeLeft)}</div>)}
                        ...
                        <div className="flex flex-col items-end">
                            {promoTimeLeft > 0 ? (
                                <><span className="text-[9px] text-gray-400 line-through mb-0.5 decoration-red-500/70 font-bold">R$ 98,00</span><button className="bg-purple-600 text-white text-xs font-black px-4 py-1.5 rounded-xl border border-purple-400 flex items-center shadow-lg group-hover:bg-purple-500 transition-colors">R$ 87,90</button></>
                            ) : (
                                <button className="bg-gray-900 text-purple-300 text-xs font-black px-4 py-1.5 rounded-xl border border-purple-500/50 flex items-center shadow-lg group-hover:bg-purple-600 group-hover:text-white transition-colors">R$ 98,00</button>
                            )}
                        </div>
                    </div>
                </div>
                <div className="p-4 bg-gray-950/50 text-center text-[10px] text-gray-500 flex justify-center items-center border-t border-white/5"><CreditCard className="w-3 h-3 mr-1.5" /> Pagamento Seguro via Mercado Pago</div>
            </div>
        </div>
      )}
      ...
      {showPaymentSuccess && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-in fade-in duration-500">
            <div className="bg-gray-900 border border-emerald-500/50 rounded-3xl w-full max-w-sm relative shadow-[0_0_50px_rgba(16,185,129,0.3)] animate-in zoom-in-95 overflow-hidden text-center p-6 sm:p-8">
                <div className="w-20 h-20 bg-emerald-500/20 rounded-full flex items-center justify-center mx-auto mb-5 animate-bounce"><CheckCircle className="w-10 h-10 text-emerald-400" /></div>
                <h2 className="text-2xl sm:text-3xl font-black text-white mb-2">Pagamento Aprovado!</h2>
                <p className="text-gray-400 text-xs sm:text-sm mb-6 font-medium">O seu saldo foi atualizado com sucesso.</p>
                <div className="bg-gray-950/50 rounded-2xl p-4 mb-6 border border-gray-800">
                    <p className="text-[10px] text-gray-500 uppercase font-bold tracking-widest mb-1">Você Recebeu</p>
                    <p className="text-4xl font-black text-amber-400 drop-shadow-md">+{lastPaymentAmount} <span className="text-2xl">🪙</span></p>
                </div>
                <button onClick={() => setShowPaymentSuccess(false)} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3.5 rounded-xl transition-all shadow-lg hover:shadow-emerald-500/30 uppercase tracking-widest text-xs">Maravilha! Continuar</button>
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full bg-emerald-500/5 blur-[100px] pointer-events-none -z-10"></div>
            </div>
        </div>
      )}
        ...
        <div className="fixed bottom-4 left-4 right-4 sm:left-auto sm:right-4 sm:w-80 bg-gray-900/95 backdrop-blur-xl border border-purple-500/40 p-4 rounded-2xl shadow-[0_0_30px_rgba(168,85,247,0.2)] z-[90] animate-in slide-in-from-bottom-10 fade-in duration-700">
           <div className="flex justify-between items-start">
              <div className="flex gap-3">
                 <div className="bg-purple-500/20 p-2 rounded-full h-fit"><Sparkles className="w-5 h-5 text-purple-400 animate-pulse" /></div>
                 <div>
                    ...
                    <button onClick={() => { setActiveLottery(drawReminder.type); setDrawReminder(null); }} className="mt-2 text-xs font-black text-purple-300 hover:text-white uppercase tracking-wider underline decoration-purple-500/50 underline-offset-4">Sim, Gerar Agora</button>
                 </div>
              </div>
      ...
      <div className="absolute top-4 left-4 sm:top-6 sm:left-6 z-50 animate-in fade-in slide-in-from-top-4">
        <button onClick={() => setShowReferral(true)} className="flex items-center bg-gray-900/90 border border-purple-500/30 rounded-full px-3 py-1.5 shadow-[0_5px_15px_rgba(0,0,0,0.5)] hover:border-purple-400 transition-all group backdrop-blur-md">
            ...
            <span className="text-purple-300 font-bold text-xs sm:text-sm drop-shadow-[0_0_5px_rgba(168,85,247,0.8)]">Indique e Ganhe</span>
        </button>
        <button onClick={() => setShowSavedGames(true)} className="flex items-center bg-gray-900/90 border border-emerald-500/30 rounded-full px-3 py-1.5 shadow-[0_5px_15px_rgba(0,0,0,0.5)] hover:border-emerald-400 transition-all group backdrop-blur-md mt-2">
            ...
            <span className="text-emerald-300 font-bold text-xs sm:text-sm drop-shadow-[0_0_5px_rgba(16,185,129,0.8)]">Meus Jogos</span>
        </button>
      </div>
      ...
      <div className="absolute top-4 right-4 sm:top-6 sm:right-6 z-50 animate-in fade-in slide-in-from-top-4 flex flex-col items-end gap-2">
        {user ? (
             <button onClick={handleLogout} className="flex items-center bg-red-900/80 border border-red-500/30 rounded-full px-3 py-1.5 shadow-lg hover:border-red-400 transition-all backdrop-blur-md text-[10px] text-red-200 font-bold uppercase tracking-wider"><LogOut className="w-3 h-3 mr-1.5" /> Sair</button>
        ) : (
             <button onClick={handleLogin} className="flex items-center bg-blue-600/90 border border-blue-400/50 rounded-full px-4 py-1.5 shadow-lg hover:bg-blue-500 transition-all backdrop-blur-md text-xs text-white font-bold uppercase tracking-wider"><LogIn className="w-3 h-3 mr-1.5" /> Entrar com Google</button>
        )}
        <button onClick={() => setShowStore(true)} className="flex items-center bg-gray-900/90 border border-amber-500/30 rounded-full pl-4 pr-1.5 py-1.5 shadow-[0_5px_15px_rgba(0,0,0,0.5)] hover:border-amber-400 transition-all group backdrop-blur-md">
            ...
            <span className="text-amber-400 font-black mr-2.5 text-sm sm:text-base drop-shadow-[0_0_5px_rgba(251,191,36,0.8)]">{coins} <span className="text-xs">🪙</span></span>
            <div className="bg-gradient-to-br from-amber-400 to-amber-600 text-amber-950 rounded-full w-7 h-7 flex items-center justify-center font-black group-hover:scale-110 transition-transform shadow-inner"><PlusCircle className="w-4 h-4" /></div>
        </button>
      </div>
      ...
      <div className="relative bg-gray-950/80 backdrop-blur-2xl border border-white/10 p-6 sm:p-10 rounded-[2rem] shadow-[0_0_50px_rgba(0,0,0,0.5)] w-full max-w-2xl text-center z-10 mt-16 sm:mt-0">
        ...
        <div className="flex flex-col items-center mb-10 relative group w-full">
          <div className="flex flex-col items-center justify-center w-full">
            <div className="rounded-2xl bg-gradient-to-br from-purple-900 to-indigo-950 border border-purple-500/30 shadow-[0_0_20px_rgba(168,85,247,0.3)] mb-6 group-hover:scale-[1.02] transition-transform duration-500 overflow-hidden w-full h-40 sm:h-56">
                <video src={magoVideo} autoPlay loop muted playsInline className="w-full h-full object-cover pointer-events-none" />
            </div>
            <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-white via-purple-100 to-purple-300 leading-none flex items-center">Oráculo da Sorte<span className={`ml-2 ${currentConfig.accent}`}>IA</span></h1>
            <div className="h-px w-32 bg-gradient-to-r from-transparent via-purple-500/50 to-transparent mt-4 mb-3"></div>
            <p className="text-[10px] sm:text-xs text-purple-300/80 font-black tracking-widest uppercase text-center">A IA que une Intuição e Matemática para decifrar as Loterias</p>
          </div>
        </div>
        ...
        <div className="flex justify-center space-x-2 mb-6 bg-gray-900/50 p-2 rounded-2xl border border-gray-800/50">
          {Object.entries(lotteriesConfig).map(([key, config]) => (
            <button key={key} onClick={() => handleLotteryChange(key)} className={`flex-1 py-3 rounded-xl text-xs sm:text-sm font-black transition-all duration-100 tab-chip ${activeLottery === key ? 'bg-gradient-to-t from-gray-700 to-gray-600 text-white shadow-[inset_0_2px_4px_rgba(255,255,255,0.1),_0_0_10px_rgba(0,0,0,0.5)] active border-gray-800' : 'bg-gray-800 border-gray-900 text-gray-500 hover:text-gray-300 hover:bg-gray-700'}`}>{config.name}</button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
            <div className={`bg-gray-900/40 p-3.5 rounded-2xl flex flex-col justify-center border border-gray-800/60 shadow-inner relative overflow-hidden transition-all`}>
                <div className="flex justify-between items-center mb-1.5 z-10"><span className="text-[9px] text-gray-500 uppercase tracking-widest font-bold">Volume de Jogos</span></div>
                <select value={numberOfGames} onChange={(e) => setNumberOfGames(Number(e.target.value))} disabled={isAiLoading || isGenerating} className={`bg-gray-950 font-semibold border border-gray-700/50 rounded-xl p-2 focus:outline-none text-sm z-10 relative text-gray-200 focus:border-gray-500`}>
            ...
            <div className={`p-3.5 rounded-2xl flex flex-col justify-center border shadow-inner relative overflow-hidden transition-all duration-500 ${currentLevelInfo.bg} ${currentLevelInfo.border}`}>
                <div className="flex justify-between items-center mb-1.5 z-10">
                   ...
                   <span className={`text-[8px] uppercase font-black px-1.5 py-0.5 rounded border ${currentLevelInfo.color} border-current opacity-80`}>{currentLevelInfo.level}</span>
                </div>
                <select value={betCounts[activeLottery]} onChange={handleBetCountChange} disabled={isAiLoading || isGenerating} className={`bg-gray-950/80 font-bold border border-gray-700/50 rounded-xl p-2 focus:outline-none text-sm z-10 relative ${currentLevelInfo.color}`}>
             ...
             <div className="p-4 sm:p-6 bg-gray-900/90 backdrop-blur-md rounded-[1.4rem] border border-purple-500/30 shadow-2xl text-left relative overflow-hidden">
                {isAiLoading ? (
                   <div className="flex flex-col items-center justify-center py-12 space-y-6">
                     {aiPersona === 'mystic' ? (
                        <><div className="relative" style={{ animation: 'astral-float 4s ease-in-out infinite' }}><div className="absolute inset-0 bg-fuchsia-500 blur-[30px] opacity-70 rounded-full animate-pulse"></div><div className="absolute inset-0 bg-purple-600 blur-[15px] opacity-90 rounded-full mix-blend-screen"></div><Sparkles className="w-20 h-20 text-fuchsia-100 relative z-10 animate-spin-slow" style={{ animationDuration: '6s' }} /></div><div className="flex flex-col items-center" style={{ animation: 'mediumnic-fade 2.5s ease-in-out infinite' }}><p className="text-fuchsia-200 font-black uppercase tracking-widest text-sm sm:text-base">Consultando os Astros...</p><p className="text-[11px] text-fuchsia-300/80 text-center mt-1">A decodificar a numerologia do seu sonho...</p></div></>
                     ) : (
                        <><div className="relative overflow-hidden rounded-2xl bg-blue-950/60 p-6 border border-blue-500/50 shadow-[0_0_30px_rgba(59,130,246,0.3)]"><div className="absolute top-0 left-0 w-full h-1 bg-cyan-400 blur-md animate-[translate-y-full_1.5s_infinite]"></div><Cpu className="w-14 h-14 text-cyan-400 relative z-10 animate-pulse" /></div><div className="flex flex-col items-center"><p className="text-cyan-300 font-black uppercase tracking-widest text-sm sm:text-base animate-pulse">A processar matrizes LLM...</p><p className="text-[11px] text-cyan-400/60 text-center mt-1">Cruzando milhares de combinações matemáticas...</p></div></>
                     )}
                   </div>
                ) : (
                  <>
                    <div className="flex bg-gray-950/80 rounded-xl p-1.5 mb-5 border border-gray-800/80 shadow-inner items-center">
                        <button onClick={() => setAiPersona('mystic')} className={`flex-1 py-2.5 text-xs font-black uppercase tracking-wider rounded-lg transition-all duration-300 ${aiPersona === 'mystic' ? 'bg-gradient-to-r from-purple-700 to-fuchsia-600 text-white shadow-[0_4px_10px_rgba(168,85,247,0.4)] scale-[1.02]' : 'text-gray-500 hover:text-gray-300'}`}>🌙 Oráculo</button>
                        <button onClick={() => setAiPersona('statistician')} className={`flex-1 py-2.5 text-xs font-black uppercase tracking-wider rounded-lg transition-all duration-300 ${aiPersona === 'statistician' ? 'bg-gradient-to-r from-blue-700 to-cyan-600 text-white shadow-[0_4px_10px_rgba(59,130,246,0.4)] scale-[1.02]' : 'text-gray-500 hover:text-gray-300'}`}>🧮 Estatístico</button>
                    </div>
                    <div className={`w-full bg-gray-950 border rounded-xl p-2 transition-all shadow-inner flex flex-col ${aiPersona === 'mystic' ? 'border-purple-500/30 focus-within:border-purple-500/70' : 'border-blue-500/30 focus-within:border-blue-500/70'}`}>
                        <label className="text-[9px] text-gray-500 font-bold uppercase tracking-widest mb-1.5 px-2 pt-1">{aiPersona === 'mystic' ? 'Qual a sua intuição ou sonho hoje?' : 'Dê uma instrução para a IA (Opcional)'}</label>
                        <textarea rows="3" value={userDream} onChange={(e) => setUserDream(e.target.value)} placeholder={aiPersona === 'mystic' ? "Ex: Sonhei com moedas de ouro e um cavalo..." : "Ex: Priorize números ímpares..."} className="w-full bg-transparent p-2 text-sm text-gray-200 placeholder-gray-700 focus:outline-none resize-none" />
                        <div className="flex items-center justify-between px-1 pb-1 pt-3 border-t border-gray-800/50 mt-1">
                            <div className="flex space-x-2">
                                <div className="flex items-center text-[9px] font-bold text-emerald-400 bg-emerald-400/10 px-2 py-1 rounded border border-emerald-400/20" title="Base de dados oficial da Caixa conectada"><Database className="w-3 h-3 mr-1" /> Dados Oficiais da Caixa</div>
                                <button onClick={() => setShowStats(!showStats)} className="flex items-center text-[9px] font-bold text-gray-400 bg-gray-800 hover:bg-gray-700 px-2 py-1 rounded border border-gray-700 transition-colors"><BarChart2 className="w-3 h-3 mr-1" />{showStats ? 'Ocultar Matrizes' : 'Ver Matrizes'}</button>
                            </div>
                    ...
                    {showStats && hasRealData && (
                      <div className="w-full text-left bg-gray-950/80 rounded-xl p-4 mt-3 overflow-y-auto max-h-[250px] custom-scrollbar border border-gray-800 shadow-inner animate-in slide-in-from-top-2">
                        <h3 className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-3 flex items-center border-b border-gray-800 pb-2"><TrendingUp className="w-3 h-3 mr-1.5" /> Raio-X do Algoritmo</h3>
                        ...
                        {Array.from({ length: currentConfig.count }).map((_, i) => {
                            ...
                            <div key={i} className="bg-gray-900/50 rounded-lg p-2.5 border border-gray-800/80">
                                <div className="flex justify-between items-center mb-1.5">
                                    <div className="flex items-center">
                                        <div className="w-5 h-5 rounded-full flex items-center justify-center font-black tracking-tighter text-[10px] bg-gradient-to-br from-gray-700 to-gray-900 text-gray-300 border border-gray-600 shadow-inner mr-1.5">{i + 1}</div>
                                        <span className="text-[9px] font-bold text-gray-400 uppercase">(De {minPossible} a {maxPossible})</span>
                                    </div>
                                ...
                                <div className="flex justify-between text-[9px] font-medium">
                                  <div className="w-1/2 pr-1"><span className="text-red-400 flex items-center gap-1 mb-0.5 font-bold">🔥 Quentes</span><span className="text-gray-300">{quentes.join(', ') || '-'}</span></div>
                                  <div className="w-1/2 pl-1 border-l border-gray-800 text-right"><span className="text-blue-400 flex items-center justify-end gap-1 mb-0.5 font-bold">🧊 Frios</span><span className="text-gray-300">{frios.join(', ') || '-'}</span></div>
                                </div>
                            </div>
                            );
                        })}
                    ...
                    <button onClick={generateGameWithAI} disabled={isAiLoading || (revealedIndex >= 0 && generatedGames.length > 0 && revealedIndex <= Math.max(...generatedGames.map(g => g.numbers.length)))} className={`w-full mt-4 py-4 rounded-2xl flex justify-center items-center text-sm sm:text-base font-black tracking-widest uppercase ${(isAiLoading || (revealedIndex >= 0 && revealedIndex <= 15)) ? 'bet-btn-disabled' : (aiPersona === 'mystic' ? 'bet-btn-mystic' : 'bet-btn-stat')}`}>
                      {aiPersona === 'mystic' ? '✨ Consultar o Plano Astral' : '🧮 Gerar com I.A.'}
                      <span className="ml-3 text-[10px] font-black bg-black/60 text-yellow-300 px-2.5 py-1 rounded-md flex items-center shadow-[inset_0_2px_4px_rgba(0,0,0,0.8)] border border-white/10 normal-case tracking-normal">{aiCost} 🪙</span>
                    </button>
                  </>
                )}
             </div>
        ...
        <div className="mb-10 min-h-[6rem]">
          {generatedGames.length === 0 && !isGenerating && revealedIndex === -1 ? (
            <div className="text-gray-600 text-sm font-medium flex flex-col items-center justify-center h-24 bg-gray-900/30 rounded-2xl border border-gray-800/50 border-dashed"><Sparkles className="w-5 h-5 mb-2 opacity-20" /> O Oráculo aguarda o seu comando...</div>
          ) : isGenerating ? (
            <div className="flex flex-col items-center justify-center h-32 bg-gray-900/30 rounded-2xl border border-gray-800/50"><RefreshCw className={`w-8 h-8 animate-spin mb-3 ${currentConfig.accent}`} /><span className="text-xs font-bold text-gray-400 uppercase tracking-widest animate-pulse">A processar...</span></div>
          ) : (
            <div className="space-y-6 max-h-[600px] overflow-y-auto custom-scrollbar pr-2 pb-4">
                {generatedGames.map((gameObj, gameIndex) => {
                    ...
                    const planTagColors = { 'Gerador Rápido': 'text-gray-400 bg-gray-800 border-gray-700', 'IA Avançada': 'text-purple-400 bg-purple-950/50 border-purple-500/50' };
                    return (
                      <div key={gameIndex} className={`bg-gray-900/60 p-6 sm:p-8 rounded-[2rem] border relative group transition-all duration-500 ${isGameFullyRevealed && alreadyDrawn ? 'border-red-500/50 shadow-[0_0_30px_rgba(239,68,68,0.1)]' : (isGameFullyRevealed && isAi ? (gameObj.aiPersonaUsed === 'statistician' ? 'border-blue-500/40 shadow-[0_0_30px_rgba(59,130,246,0.1)]' : 'border-purple-500/40 shadow-[0_0_30px_rgba(168,85,247,0.1)]') : 'border-gray-700/50')}`}>
                          {isGameFullyRevealed && alreadyDrawn && (<div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-red-600 text-white text-[9px] font-black uppercase tracking-widest px-4 py-1.5 rounded-full shadow-[0_0_15px_rgba(220,38,38,0.5)] flex items-center z-10 animate-in zoom-in duration-500"><ShieldAlert className="w-3 h-3 mr-1.5 animate-pulse" /> Combinação Já Sorteada!</div>)}
                          <div className="flex justify-between items-center mb-6">
                              <div className="flex flex-wrap items-center gap-2">
                                {isAi ? (
                                  <span className={`text-[9px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full border shadow-inner flex items-center transition-colors ${isGameFullyRevealed ? (gameObj.aiPersonaUsed === 'statistician' ? 'text-blue-300 bg-blue-950/50 border-blue-500/30' : 'text-purple-300 bg-purple-950/50 border-purple-500/30') : 'text-gray-500 bg-gray-900 border-gray-700'}`}>{gameObj.aiPersonaUsed === 'statistician' ? <><Cpu className="w-3 h-3 mr-1"/> IA Estrategista</> : <><Sparkles className="w-3 h-3 mr-1"/> Oráculo IA</>}</span>
                                ) : (
                                  <span className="text-[9px] font-black uppercase tracking-widest text-gray-400 bg-gray-800 px-3 py-1.5 rounded-full border border-gray-700 shadow-inner flex items-center"><Dices className="w-3 h-3 mr-1" /> Bilhete {gameIndex + 1}</span>
                                )}
                                <span className={`text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded border shadow-sm flex items-center ${gameLevelInfo.color} ${gameLevelInfo.bg} ${gameLevelInfo.border}`}>{game.length > currentConfig.count ? <Zap className="w-3 h-3 mr-1" /> : ''} {gameLevelInfo.title}</span>
                                {isGameFullyRevealed && (<span className={`text-[8px] font-bold uppercase tracking-widest px-2 py-1 rounded border shadow-sm flex items-center animate-in fade-in ${planTagColors[gameObj.gameType]}`}>{gameObj.gameType}</span>)}
                              </div>
                              {isGameFullyRevealed && (
                                <div className="flex items-center gap-2 shrink-0">
                                  <button onClick={() => copyToClipboard(game)} className="p-2 text-gray-500 hover:text-white bg-gray-800/50 hover:bg-gray-700 rounded-lg transition-colors animate-in fade-in duration-300" title="Copiar"><Copy className="w-4 h-4" /></button>
                                  <button onClick={() => handleShareWhatsApp(gameObj, gameIndex)} className="p-2 text-gray-500 hover:text-green-400 bg-gray-800/50 hover:bg-gray-700 rounded-lg transition-colors animate-in fade-in duration-300" title="Compartilhar no WhatsApp"><Share2 className="w-4 h-4" /></button>
                                  <button onClick={() => handleSaveGame(gameObj)} className="p-2 text-gray-500 hover:text-emerald-400 bg-gray-800/50 hover:bg-gray-700 rounded-lg transition-colors animate-in fade-in duration-300" title="Salvar na Conta"><Save className="w-4 h-4" /></button>
                                </div>
                              )}
                          </div>
                          <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
                              {game.map((num, index) => {
                                 const ballStyle = alreadyDrawn ? 'bg-[radial-gradient(circle_at_30%_30%,_#fca5a5,_#ef4444,_#991b1b)] text-white shadow-[inset_-2px_-2px_4px_rgba(0,0,0,0.5),_0_0_25px_rgba(239,68,68,0.85)] border border-red-300' : 'bg-[radial-gradient(circle_at_30%_30%,_#fef08a,_#eab308,_#854d0e)] text-amber-950 shadow-[inset_-2px_-2px_6px_rgba(0,0,0,0.6),_0_0_25px_rgba(251,191,36,0.9)] border border-yellow-200';
                                 const sizeClass = game.length > 10 ? 'w-9 h-9 sm:w-11 sm:h-11 text-sm sm:text-base' : 'w-12 h-12 sm:w-14 sm:h-14 text-lg sm:text-xl';
                                 return (<LotteryBall key={index} finalNumber={num} index={index} revealedIndex={revealedIndex} ballStyle={ballStyle} sizeClass={sizeClass} maxVal={currentConfig.max} />);
                              })}
                          </div>
                          {isGameFullyRevealed && isAi && gameObj.aiMessage && (
                             <div className={`mt-8 pt-5 border-t text-sm flex flex-col sm:flex-row gap-4 items-start text-left p-4 rounded-xl animate-in slide-in-from-top-4 duration-500 ${gameObj.aiPersonaUsed === 'statistician' ? 'border-blue-500/20 text-blue-200/90 bg-blue-950/30' : 'border-purple-500/20 text-purple-200/90 bg-purple-950/30'}`}>
                                <div className="flex-1 flex items-start">
                                  {gameObj.aiPersonaUsed === 'statistician' ? <Calculator className="w-4 h-4 mr-2.5 mt-0.5 shrink-0 text-blue-400" /> : <MessageCircle className="w-4 h-4 mr-2.5 mt-0.5 shrink-0 text-purple-400" />}
                                  <span className="italic leading-relaxed text-xs sm:text-sm font-medium">{gameObj.aiMessage}</span>
                                </div>
                                <button onClick={() => handlePlayAudio(gameObj.aiMessage, gameIndex, gameObj.aiPersonaUsed)} disabled={isAudioLoading} className={`shrink-0 flex items-center justify-center py-2 px-4 rounded-lg text-xs font-bold transition-all border shadow-lg ${gameObj.aiPersonaUsed === 'statistician' ? 'bg-blue-900/60 hover:bg-blue-800 text-blue-100 border-blue-500/40 shadow-[0_0_10px_rgba(59,130,246,0.2)]' : 'bg-purple-900/60 hover:bg-purple-800 text-purple-100 border-purple-500/40 shadow-[0_0_10px_rgba(168,85,247,0.2)]'}`}>
                                  {isAudioLoading && !audioData[gameIndex] ? <><RefreshCw className="w-3.5 h-3.5 mr-2 animate-spin"/> Sintetizando...</> : <><Volume2 className="w-3.5 h-3.5 mr-2"/> Ouvir Oráculo</>}
                                </button>
                             </div>
                          )}
                      </div>
                ...
                {allGamesRevealed && (
                    <div className="mt-8 flex flex-col items-center justify-center animate-in fade-in zoom-in duration-500">
                       <button onClick={handleSendEmail} disabled={isSendingEmail || emailSent} className={`py-3.5 px-6 rounded-2xl font-bold text-xs sm:text-sm flex items-center transition-all shadow-lg border ${emailSent ? 'bg-emerald-900/40 text-emerald-400 border-emerald-500/50 cursor-default' : 'bg-gray-800 hover:bg-gray-700 text-gray-200 border-gray-600 hover:border-gray-500'}`}>
        ...
        <div className="pt-6 border-t border-gray-800/80 flex flex-col items-center space-y-4">
            <button onClick={generateGames} disabled={isGenerating || (revealedIndex >= 0 && generatedGames.length > 0 && revealedIndex <= Math.max(...generatedGames.map(g => g.numbers.length)))} className={`w-full max-w-sm py-4 rounded-2xl font-black text-xs sm:text-sm uppercase tracking-widest flex items-center justify-center ${(isGenerating || (revealedIndex >= 0 && revealedIndex <= 15)) ? 'bet-btn-disabled' : 'bet-btn-gold'}`}>
              <span>Gerar Sorteio Rápido (Sem IA)</span>
              {revealedIndex < 0 && !isGenerating && (<span className="ml-3 text-[10px] font-black bg-black/60 text-yellow-300 px-2 py-0.5 rounded-md flex items-center shadow-[inset_0_2px_4px_rgba(0,0,0,0.8)] border border-white/10 normal-case tracking-normal">{currentCost} 🪙</span>)}
            </button>
